...
function ProjectSpear({ phase, focusIndex }: ProjectSpearSceneProps) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    spear.current.rotation.z = MathUtils.damp(spear.current.rotation.z, targetRotationZ, damping, delta)
    spear.current.rotation.y = MathUtils.damp(spear.current.rotation.y, targetRotationY, damping, delta)
    spear.current.position.x = MathUtils.damp(spear.current.position.x, targetX, damping, delta)
    spear.current.position.y = MathUtils.damp(spear.current.position.y, targetY, damping, delta)
    spear.current.scale.setScalar(MathUtils.damp(spear.current.scale.x, targetScale, damping, delta))
  })
